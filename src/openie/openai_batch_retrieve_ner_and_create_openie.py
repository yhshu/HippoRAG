...
if __name__ == '__main__':
    ...
    openie_submission_jsonl_path = f'output/openai_batch_submission_openie_{args.dataset}_{args.model_name}.jsonl'
    ...
    if not skip_openie:
        ...
        with open(openie_submission_jsonl_path, 'w') as f:
            ...
            print("Batch file saved to", openie_submission_jsonl_path, 'len: ', len(openie_submission_jsonl))

        batch_input_file = client.files.create(file=open(openie_submission_jsonl_path, 'rb'), purpose='batch')
        batch_obj = client.batches.create(input_file_id=batch_input_file.id, endpoint='/v1/chat/completions',
                                          completion_window='24h',
                                          metadata={'description': f"HippoRAG OpenIE for {args.dataset}, len: {len(openie_submission_jsonl)}"})
        print(batch_obj)
        print()

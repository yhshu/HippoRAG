...
def openie_for_one_batch(corpus_path, extractor_name):
    corpus_label = corpus_path.split('/')[-1].split('.')[0]
    ...
    ner_responses, ner_total_tokens = named_entity_recognition_batch_vllm(client, passages, extractor_name)
    entities_list = []
    ...
    openie_responses, openie_total_tokens = openie_post_ner_extract_batch_vllm(client, passages, entities_list,
                                                                               extractor_name)
    triples = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--start', type=int, default=0, help='Start index of the batch')
    parser.add_argument('--end', type=int, default=100, help='End index of the batch')
    ...
    for i in range(args.start, args.end):
        corpus_path = f'data/corpus_batch/musique_{i}_corpus.json'
        openie_for_one_batch(corpus_path, args.extractor)

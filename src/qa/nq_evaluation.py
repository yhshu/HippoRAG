...
import nq_evaluation_util as util
import six
...
flags.DEFINE_string(
    "gold_path",
    None,
    "Path to the gzip JSON data. For "
...
def score_long_answer(gold_label_list, pred_label):
    ...
    if gold_has_answer and pred_has_answer:
        for gold_label in gold_label_list:
            ...
            if util.nonnull_span_equal(
                gold_label.long_answer_span, pred_label.long_answer_span
            ):
                is_correct = True
                break
...
def score_short_answer(gold_label_list, pred_label):
    ...
    if gold_has_answer and pred_has_answer:
        if pred_label.yes_no_answer != "none":  # System thinks its y/n questions.
            ...
            for gold_label in gold_label_list:
                if util.span_set_equal(
                    gold_label.short_answer_span_list, pred_label.short_answer_span_list
                ):
                    is_correct = True
                    break
...
def print_r_at_p_table(answer_stats):
    ...
    for target, recall, precision, row in pr_table:
        print(
            "R@P={}: {:.2%} (actual p={:.2%}, score threshold={:.4})".format(
...
def read_annotation_json(gold_path):
    annotation_dict = {}
    gold_data = json.load(open(gold_path))
    for json_example in gold_data:
...
def collect_annotations(spans: set, annotation: dict, document_tokens):
    ...
    span = []
    if start_byte != -1 and end_byte != -1:
        for token in document_tokens:
            if start_byte <= token["start_byte"] and token["end_byte"] <= end_byte and token['html_token'] is False:
                span.append(token["token"])
        if len(span) > 0:
            spans.add(" ".join(span))
    elif start_token != -1 and end_token != -1:
        for i, token in enumerate(document_tokens):
            if start_token <= i and i <= end_token and token['html_token'] is False:
                span.append(token["token"])
        if len(span) > 0:
            spans.add(" ".join(span))
    return spans
...
def evaluate_with_references(predictions, references):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log", type=str, help="Path to the prediction data")
    ...
    for sample in data:
        ...
        annotated_short_answers = set()
        for annotation in sample["annotations"]:
            if "long_answer" in annotation:
                # find the annotated answer in sample['document_tokens']
                long_answer = annotation["long_answer"]
                annotated_long_answers = collect_annotations(annotated_long_answers, long_answer, sample["document_tokens"])
            if "short_answer" in annotation:
                short_answer = annotation["short_answer"]
                annotated_short_answers = collect_annotations(annotated_short_answers, short_answer, sample["document_tokens"])

        prediction = sample["prediction"]
        annotated = False
        # if len(annotated_long_answers):
        #     print(annotated_long_answers)
        #     print(prediction)
        #     annotated = True

        if len(annotated_short_answers):
            print(annotated_short_answers)
            print(prediction)
            annotated = True

        if not annotated:
            print("No annotated answers")
            continue

        predictions.append(prediction)
        long_references.append(list(annotated_long_answers))
        short_references.append(list(annotated_short_answers))

    evaluate_with_references(predictions, long_references)
    evaluate_with_references(predictions, short_references)

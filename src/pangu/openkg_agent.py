...
from src.pangu.environment.environment import Env
from src.pangu.environment.examples.KB.OpenKGEnv import lisp_to_sparql, OpenKGEnv
from src.pangu.language.domain_language import DomainLanguage
from src.pangu.language.openkg_language import OpenKGLanguage
from src.pangu.language.plan_wrapper import Plan
from src.pangu.query_util import execute_query_with_virtuoso, lisp_to_label


class OpenKGAgent:
    def __init__(self, language: DomainLanguage, environment: Env = None, find_new_elements=False):
    ...
    def add_class_to_action(self, action):
        ...
        new_query = '\n'.join(new_clauses)
        try:
            rows = execute_query_with_virtuoso(new_query)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mid_to_node', type=str, default='output/ttl_kg/musique_facts_and_sim_graph_ents_only_lower_preprocess_ner_facebook_contriever.v3.mid_to_node.json')
    parser.add_argument('--node_to_mid', type=str, default='output/ttl_kg/musique_facts_and_sim_graph_ents_only_lower_preprocess_ner_facebook_contriever.v3.node_to_mid.json')
    args = parser.parse_args()

    mid_to_node = json.load(open(args.mid_to_node))
    language = OpenKGLanguage()
    environment = OpenKGEnv()
    agent = OpenKGAgent(language=language, environment=environment, find_new_elements=True)

    agent.initialize_plans({"Node": {Plan("https://github.com/OSU-NLP-Group/HippoRAG/musique/Node9969")}})
    step = 1
    while step <= 4:
        ...
        for rtn_type in new_plans:
            ...
            for plan in new_plans[rtn_type]:
                print({'plan': plan, 'plan_str': lisp_to_label(plan.plan, mid_to_node)})
            print()
